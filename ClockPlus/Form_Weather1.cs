...
namespace ClockPlus
{
    public partial class Form_Weather1 : Form
    {
        ...
        {
            // 位置・不透明度・最前面の設定
            this.Location = new System.Drawing.Point(XML_Main.cnf.Weather.Weather_Disp[0].PosX, XML_Main.cnf.Weather.Weather_Disp[0].PosY);
            this.Opacity = XML_Main.cnf.Weather.Weather_Disp[0].Opacity;
            this.TopMost = XML_Main.cnf.Weather.Weather_Disp[0].TopMost;

            // 倍率設定
            Weather_Disp1_Size(App.form_weather1);     
        ...
        // 倍率設定
        static public void Weather_Disp1_Size(System.Windows.Forms.Form form)
        {
            int Wid;
            int Hei;
            Wid = (int)(Width * (XML_Main.cnf.Weather.Weather_Disp[0].Zoom));
            Hei = (int)(Height * (XML_Main.cnf.Weather.Weather_Disp[0].Zoom));
            System.Drawing.Size Form_size = new System.Drawing.Size(Wid, Hei);
            form.Size = Form_size;
        }

        // 今日の天気予報を描画
        public void Weather_Disp1_Drow()
        {
            Bitmap canvas = null;

            // 描画先とするオブジェクトを作成する
            canvas = new Bitmap(Form_Weather1.Width, Form_Weather1.Height);
            ...
            // 文字の色
            Brush Text_Color = new SolidBrush(Etc.Color_Code(XML_Main.cnf.Weather.Weather_Disp[0].Font_Color));     // メイン
            Brush Tmpmax_Color = new SolidBrush(Etc.Color_Code(XML_Main.cnf.Weather.Weather_Disp[0].Font_Color1));  // 最高気温 
            Brush Tmpmin_Color = new SolidBrush(Etc.Color_Code(XML_Main.cnf.Weather.Weather_Disp[0].Font_Color2));  // 最低気温 
            ...
            if (tenki != null)
            {
                ...
                forecast = WeatherHacks.tenki.forecasts[0];
            ...
            if (XML_Main.cnf.Weather.Weather_Disp[0].Clear_Background)
            {
                PictureBox_Form.BackColor = Color.Transparent;    // 背景色を透過にする
            }
            else
            {
                PictureBox_Form.BackColor = Etc.Color_Code(XML_Main.cnf.Weather.Weather_Disp[0].Back_Color);
            }
        ...
        {
            XML_Main.cnf.Weather.Weather_Disp[0].Enable = false;
        }
        ...
        private void Panel_MouseMove(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                // フォームの位置を取得する
                XML_Main.cnf.Weather.Weather_Disp[0].PosX = point.X;
                XML_Main.cnf.Weather.Weather_Disp[0].PosY = point.Y;
            }
        }
    }
}
